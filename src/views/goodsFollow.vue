<template>
  <div class="page pb-11">
    <iHeader @back="$router.back()" text="我的关注" @right="editFlag=!editFlag" :rightText="editFlag?'完成':'编辑'" />
    <van-list
      class="px-2 py-4"
      v-if="list.length"
...
export default {
  ...
  methods: {
    async getList (e) {
      ...
      const res = await this.$http.get('/user/findGoodsFollowPage', { params })
      if (res.data.success) {
    ...
    async deleteCertain () {
      ...
      const data = {
        followIds: this.goodsIdList
      }
      const res = await this.$http.post('/user/deleteGoodsFollow', data)
      if (res.data.success) {
