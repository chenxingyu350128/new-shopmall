<template>
  <div class="page grey lighten-2 pb-11">
    ...
    <div
      class="primary white--text px-4 py-2 mb-1 d-flex align-center justify-space-between"
    >
      <div>
    ...
    <div class="white pa-2 mb-1 d-flex align-center justify-space-between">
      ...
      <div class="flex-fill caption mx-2">
        ...
        <div class="d-flex justify-space-between">
          ...
          <span class="ml-2"
            >{{ region.split("-").join("") }}{{ address }}</span
          >
        </div>
      </div>
    ...
    <div
      v-if="logisticsNum"
      class="white pa-2 mb-1 d-flex align-center justify-space-between"
    >
    ...
    <div v-for="(item, i) in orderGoodsList" :key="i" class="white pa-2 d-flex">
      ...
      <div class="ml-2 flex-fill d-flex flex-column justify-space-between">
        ...
        <div v-if="item.skuSpec" class="caption grey--text text--lighten-2">
          规格： {{ item.skuSpec }}
        </div>
        <div class="d-flex justify-space-between">
    ...
    <v-divider></v-divider>
    <div
      class="d-flex justify-space-between px-4 py-2 white caption grey--text"
    >
      <span>商品总价</span>
      <span>+￥{{ payMoney }}</span>
    </div>
    ...
    <v-divider></v-divider>
    <div
      class="d-flex justify-space-between px-4 py-2 white mb-1 subtitle-2 primary--text"
    >
      <span>需付款</span>
      <span>￥{{ totalMoney }}</span>
    </div>
    ...
    <v-footer fixed bottom class="white">
      <div v-if="dealStatus === 1" class="text-right full-width">
        <v-btn class="ml-2" small color="primary" depressed @click="payOrder"
          >付款</v-btn
        >
        <v-btn class="ml-2" small color="primary" depressed @click="cancelOrder"
          >取消订单</v-btn
        >
      </div>
      <div v-if="dealStatus === 2" class="text-right full-width">
        <v-btn class="ml-2" small color="primary" depressed @click="refund"
          >退款</v-btn
        >
        <v-btn class="ml-2" small color="primary" depressed @click="remind"
          >提醒发货</v-btn
        >
      ...
      <div v-if="dealStatus === 3" class="text-right full-width">
        <v-btn class="ml-2" small color="primary" depressed @click="refund"
          >退款</v-btn
        >
        <v-btn class="ml-2" small color="primary" depressed @click="confirmGet"
          >确认收货</v-btn
        >
      </div>
      ...
      <div v-if="dealStatus >= 5" class="text-right full-width">
        <v-btn small color="primary" depressed @click="deleteOrder"
          >删除订单</v-btn
        >
      </div>
    </v-footer>
...
export default {
      ...
      handler: function(val) {
        switch (val) {
          ...
          case 3:
            ...
            break;
          case 4:
            this.orderTopText = "订单送达";
            this.orderTopText_2 = "欢迎评价";
            break;
          case 5:
            this.orderTopText = "交易成功";
            this.orderTopText_2 = "谢谢惠顾";
            break;
          case 6:
            // case 6:
            this.orderTopText = "交易关闭";
            this.orderTopText_2 = "订单已关闭";
            break;
        }
      },
      immediate: true
  ...
  methods: {
    init() {
      ...
      this.$http.get("/order/getOrderByGoods", { params }).then(res => {
        if (res.data.success) {
