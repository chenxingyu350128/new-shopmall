<template>
  <div class="page">
    ...
    >
      <v-card
        @click="toDetail(item, item.orderId)"
      ...
      >
        <div class="pa-2">
          ...
          <div
            class="d-flex"
            v-for="(itm, idx) in item.orderGoodsList"
            :key="idx"
          >
            ...
            <div class="flex-fill ml-2">
              ...
              <div class="subtitle-1 primary--text text-right">
                {{ itm.goodsPrice }}
              </div>
              <div
                v-if="itm.skuSpec"
              ...
              <div class="subtitle-1 grey--text text-right">
                X{{ itm.goodsNum }}
              </div>
            </div>
          </div>
        ...
        <div class="pa-2 text-right">
          共{{ item.count }}件商品，合计￥{{ item.totalMoney }}(不含运费)
        </div>
        <div class="pa-2 text-right">
          <v-btn
            @click.stop="deleteOrder({ orderId: item.orderId })"
            small
            class="ml-2"
            v-if="item.dealStatus >= 5"
            color="primary"
            depressed
            >删除订单</v-btn
          >
          ...
          >
          <v-btn
            @click.stop="remind({ orderId: item.orderId })"
            small
            class="ml-2"
          ...
          >
          <v-btn
            @click.stop="confirmGet({ orderId: item.orderId })"
            small
            class="ml-2"
            v-if="item.dealStatus === 3"
            color="primary"
            depressed
            >确认收货</v-btn
          >
          <v-btn
            @click.stop="review({ item: item })"
            small
            class="ml-2"
          ...
          >
          <v-btn
            @click.stop="cancelOrder({ orderId: item.orderId })"
            small
            class="ml-2"
            v-if="item.dealStatus === 1"
            color="primary"
          ...
          >
          <v-btn
            @click.stop="refund({ orderId: item.orderId })"
            small
            class="ml-2"
            v-if="item.dealStatus === 2 || item.dealStatus === 3"
            color="primary"
            depressed
            >退款</v-btn
          >
...
export default {
  ...
  created() {
    ...
    if (hasSearch) {
      ...
      const { sessionId, type, outside } = keyvalue;
      this.outside = outside;
      this.currentIndex = type;
      this.silentLogin(sessionId, type);
      return false;
    }
    if (this.isWeChat()) {
      // 微信公众号
  ...
  mounted() {
    if (this.$route.query.type) {
      this.currentIndex = parseInt(this.$route.query.type);
    }
    console.log("outside值为", this.outside);
    if (!this.outside) {
      console.log("网页登录");
      this.init();
    }
  ...
  methods: {
    ...
    async silentLogin(sessionId, type) {
    ...
    handle(url, data) {
      ...
      this.$http.post(realUrl, data).then((res) => {
        if (res.data.success) {
          this.init();
    ...
    init(e) {
      ...
      console.log("token值为？", this.token);
