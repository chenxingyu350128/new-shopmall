<template>
  <div class="page grey lighten-3">
    ...
    <div>
      ...
      <div v-for="(item, i) in orderGoodsList" :key="i" class="white pa-2 mb-2">
        <div class="d-flex align-center mb-2">
          ...
          <div
            class="ml-4 flex-fill d-flex align-center caption justify-space-between"
          >
            <span>描述相符</span>
            <v-rating
              color="yellow"
        ...
        ></v-textarea>
        <div class="d-flex">
          <div v-if="item.images.length < 4" class="add">
          ...
          <div
            ...
            class="flex-fill d-flex justify-space-around"
          >
            <div v-for="(x, idx) in item.images" :key="idx" class="uploadPic">
              <v-avatar size="65" tile @click="visualClick(idx, item, i)">
                <img :src="x" />
              </v-avatar>
              <v-icon @click="deleteImg(idx, item)" dense color="primary"
    ...
    <v-footer
      fixed
      bottom
      class="white pa-0 justify-space-between align-center"
    >
      <div @click="isAnonymous = !isAnonymous" class="d-flex align-center px-2">
        <v-icon dense :color="isAnonymous ? 'primary' : 'grey lighten-2'">{{
          isAnonymous ? "mdi-check-circle" : "mdi-circle-outline"
        }}</v-icon>
...
export default {
  ...
  methods: {
        ...
        .then(res => {
          if (res.data.success) {
            ...
            if (!this.addClick) {
              ...
              const arr = this.editItem.images.splice(
                this.editImageIndex,
                1,
                pic
              );
    ...
    commit() {
      ...
      this.$http.post("/order/addEval", data).then(res => {
        if (res.data.success) {
